...
const ChatInput = ({ onSend, disabled }: ChatInputProps) => {
    ...
    return (
        <div className="flex flex-col gap-2">
            <div ref={dropdownRef} className="relative w-36 sm:w-40">
                <button
                    type="button"
                    onClick={() => !disabled && setOpen(o => !o)}
                    disabled={disabled}
                    className="w-full flex items-center justify-between gap-2 bg-[#2a2a2a] border border-gray-600
                               text-gray-100 text-sm rounded-xl px-3 sm:px-4 py-2.5 focus:outline-none
                ...
                {open && (
                    <div className="absolute bottom-full mb-1 left-0 w-full bg-[#2a2a2a] border border-gray-600
            ...
            <div className="flex gap-2 sm:gap-3 items-end w-full">
                <textarea
                    ...
                    rows={1}
                    className="flex-1 min-w-0 resize-none rounded-xl border border-gray-600 bg-[#2a2a2a]
                               text-gray-100 placeholder-gray-400 px-3 sm:px-4 py-3 text-sm
                               focus:outline-none focus:border-blue-500 transition-colors"
                />
                <button
                    onClick={handleSend}
                    disabled={disabled || !input.trim()}
                    className="px-4 sm:px-5 py-3 bg-blue-600 hover:bg-blue-500 disabled:opacity-40
